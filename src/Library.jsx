...
function Library() {
  ...
  // Sessions to show (all or single date)
  ...
  const generateSummary = async (session) => {
    ...
    try {
      ...
      setSummaries((prev) => ({ ...prev, [session.id]: out }));
    } catch {
          ...
          <div className="grid grid-cols-7 text-center mt-2 gap-1">
            {calendarCells.map((day, idx) => {
              const dateStr = day
                ...
                : null;
              const sess = dateStr && sessionsMap[dateStr];
              const avgImp = sess
                ? Math.round(
                    sess.entries.reduce((sum, e) => sum + e.importance, 0) /
                      sess.entries.length
                  )
                : 0;
              ...
              return (
                <div
                  ...
                  className={`relative h-12 flex flex-col justify-center items-center text-sm p-1 rounded cursor-pointer
                    ...
                    ${sess ? "bg-purple-50 dark:bg-purple-800" : ""}
                    hover:bg-gray-100 dark:hover:bg-gray-800`}
                >
                  {day}
                  {sess && (
                    <span className="absolute bottom-1 text-xs text-purple-600 dark:text-purple-300">
                      {avgImp}★
                    </span>
                  )}
                </div>
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-yellow-400">
                        {"★".repeat(e.importance) +
                          "☆".repeat(5 - e.importance)}
                      </span>
                    </div>
                    <p>
                      <strong>📝 You:</strong> {e.user}
                    </p>
                    <p className="italic text-gray-600 dark:text-gray-400">
                      <strong>🔍 Insight:</strong> {e.ai}
                    </p>
                    <button
